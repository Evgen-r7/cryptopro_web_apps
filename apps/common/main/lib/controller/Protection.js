    ...
    Common.Controllers.Protection = Backbone.Controller.extend(_.extend({
            ...
            this.addListeners({
                'Common.Views.Protection': {
                    ...
                    'protect:signature':     _.bind(this.onSignatureClick, this),
										"protect:cryptopro": _.bind(this.onCryptoproClick, this), // kzn_unit #cryptopro
                }
            });
        },
        onLaunch: function () {
        ...
        onSignatureClick: function(type, signed, guid){
            switch (type) {
				...
				// web-apps/apps/common/main/lib/controller/Protection.js
				// kzn_unit #cryptopro
				onCryptoproSign: function() {
					new DE.Views.CryptoProSign({
							handler: function(e, i) {
								console.log(arguments);
							},
							model: new DE.Models.CryptoProModel()
					}).show();
				},
				onCryptoproVerify: function() {
					function showStatus(status, isError, details) {
						// console.log('details', details);
						new DE.Views.CryptoProDialog({
								txtTitle: 'Провека подписи документа',
								txtStatus: status,
					...
					function onGetPath(file) {
						...
						let model = new DE.Models.CryptoProModel();

						try {
							let status = model.verify(file);
							let statusView = status.error ? `Документ не содержит подписи!` : `Подпись документа верна!`;
							showStatus(statusView, status.error, status.details);
							...
							showStatus('Произошла ошибка при проверке подписи!', true, details);
							return model.destroy();
						}
					...
					window.AscDesktopEditor.OpenFilenameDialog('', false, onGetPath);
				},
				onCryptoproSettings: function() {
					window.AscDesktopEditor.execCommand("kzn_unit", JSON.stringify({ "class": "ds" , "method": "exec_cryptopro"}))
				},
				onCryptoproClick: function(t, e, i) {
					switch (t) {
						case "cryptopro_sign":
								this.onCryptoproSign();
								break;
						case "cryptopro_verify":
						this.onCryptoproVerify();
						break;
					case "cryptopro_settings":
						this.onCryptoproSettings();
						break;
					}
				},
