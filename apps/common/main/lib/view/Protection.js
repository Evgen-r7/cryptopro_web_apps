        ...
        var template =
            ...
            '<div class="group">' +
                ...
                '<span id="slot-btn-signature" class="btn-slot text x-huge"></span>' +
								'<span id="slot-btn-cryptopro" class="btn-slot text x-huge"></span>' +
            '</div>' +
            '</section>';
            ...
            if (me.appConfig.isSignatureSupport) {
                ...
                this.btnsInvisibleSignature.forEach(function(button) {
                    button.on('click', function (b, e) {
						...
						// web-apps/apps/common/main/lib/view/Protection
						// kzn_unit #cryptopro
            if (me.appConfig.isCryptoproSupport) {
							if (this.btnCryptopro.menu)
									this.btnCryptopro.menu.on('item:click', function (menu, item, e) {
											me.fireEvent('protect:cryptopro', [item.value, false]);
									});

							this.btnsCryptopro.forEach(function(button) {
									button.on('click', function (b, e) {
											me.fireEvent('protect:cryptopro', ['cryptopro_sign']);
									});
							});
						}
                ...
                if (this.appConfig.isSignatureSupport) {
                    ...
                    if (!this.btnSignature.menu)
                        this.btnsInvisibleSignature.push(this.btnSignature);
								...
								// kzn_unit #cryptopro
								this.appConfig.isCryptoproSupport = true;
								if (this.appConfig.isCryptoproSupport) {
                    this.btnCryptopro = new Common.UI.Button({
                        cls: 'btn-toolbar x-huge icon-top',
                        iconCls: 'toolbar__icon btn-ic-signature',
                        caption: this.txtCryptopro,
												menu: (this.appPrefix !== 'pe-')
                    });
								}
                                    ...
                                    items: [
                                        ...
                                        },
                                        {
                                            ...
                                            disabled: me._state.disabled
												...
												// kzn_unit #cryptopro
                        if (me.btnCryptopro) {
													me.btnCryptopro.updateHint((me.btnCryptopro.menu) ? me.hintSignature : me.txtInvisibleSignature);
													me.btnCryptopro.menu && me.btnCryptopro.setMenu(
															new Common.UI.Menu({
																	items: [
																			{
																					caption: 'Подписать',
																					value: 'cryptopro_sign'
																			},
																			{
																					caption: 'Проверить',
																					value: 'cryptopro_verify'
																			}/*,
																			{
																					caption: 'Настройки',
																					value: 'cryptopro_settings'
																			}*/
																	]
															})
													);
            ...
            getPanel: function () {
                ...
                if ( this.appConfig.canProtect ) {
                    ...
                    this.btnSignature && this.btnSignature.render(this.$el.find('#slot-btn-signature'));
										...
										// web-apps/apps/common/main/lib/view/Protection
										// kzn_unit #cryptopro
                    this.btnCryptopro && this.btnCryptopro.render(this.$el.find('#slot-btn-cryptopro'));
                }
                return this.$el;
            },
            ...
            getButton: function(type, parent) {
                ...
                if ( type == 'signature' ) {
                    ...
                    return button;
									...
									this.btnsCryptopro.push(button);
									if (this._isSetEvents) {
											button.on('click', function (b, e) {
													me.fireEvent('protect:cryptopro', ['cryptopro_sign']);
											});
